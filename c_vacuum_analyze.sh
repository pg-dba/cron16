...
echo "  $(date --iso-8601=seconds) : VACUUM (ANALYZE,VERBOSE) \"${schemaName}\".\"${tableName}\";" >> ${logfile} 2>&1;
PGPASSWORD=${PASSWORD} psql -h ${HOST} -p ${PORT} -U ${USERNAME} -d ${DBs[$dbName]} -c "VACUUM (ANALYZE,VERBOSE) \"${schemaName}\".\"${tableName}\";" >> ${logfile} 2>&1;
done;
done;
done;
