...
echo "  $(date --iso-8601=seconds) : VACUUM VERBOSE ANALYZE \"${schemaName}\".\"${tableName}\";" >> ${logfile} 2>&1;
PGPASSWORD=${PASSWORD} psql -h ${HOST} -p ${PORT} -U ${USERNAME} -d ${DBs[$dbName]} -c "VACUUM VERBOSE ANALYZE \"${schemaName}\".\"${tableName}\";" >> ${logfile} 2>&1;
done;
done;
done;
echo "===== $(date --iso-8601=seconds) VACUUM ANALYZE finished =====" &>>${logfile};
echo "===== ${HOST} VACUUM ANALYZE finished =====" 2>&1;
