...
IFS="|";
echo "===== ${HOST} VACUUM started ====="  2>&1;
echo "===== $(date --iso-8601=seconds) VACUUM started ====="  > ${logfile} 2>&1;
...
echo "  $(date --iso-8601=seconds) : VACUUM VERBOSE \"${schemaName}\".\"${tableName}\";" >> ${logfile} 2>&1;
PGPASSWORD=${PASSWORD} psql -h ${HOST} -p ${PORT} -U ${USERNAME} -d ${DBs[$dbName]} -c "VACUUM VERBOSE \"${schemaName}\".\"${tableName}\";" >> ${logfile} 2>&1;
done;
done;
done;
echo "===== $(date --iso-8601=seconds) VACUUM finished =====" &>>${logfile};
echo "===== ${HOST} VACUUM finished =====" 2>&1;
